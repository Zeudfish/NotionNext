...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  const findPost = () => {
    ...
    return (
      ...
      // 2. 兼容展示 URL 带类型前缀、Notion slug 只保存末段的情况
      props.allPages.find(
        p => p && !p.type?.includes('Menu') && p.slug === lastSegment
      ) ||
      // 3. UUID 匹配
      props.allPages.find(p => p?.id === fullSlug || p?.id === lastSegment) ||
      null
    )
  }
...
async function convertNotionToSiteData(
  ...
  const customNav = normalizeFoodMenuLinks(getCustomNav({ allPages: collectionData.filter(post => post?.type === 'Page' && post.status === 'Published') }))
  const customMenu = normalizeFoodMenuLinks(getCustomMenu({ collectionData, NOTION_CONFIG }))
  const latestPosts = getLatestPosts({
    allPages,
    from,
    latestPostCount: siteConfig('LATEST_POST_COUNT', 6, NOTION_CONFIG)
  })
  const allNavPages = normalizeFoodMenuLinks(getNavPages({ allPages }))
  const allLinkPages = normalizeFoodMenuLinks(getLinkPages({ allPages }))
...
function normalizeFoodMenuLinks(items) {
  if (!Array.isArray(items)) return items
  return items.map(item => {
    const normalized = { ...item }
    if (normalized.href === '/shanghai-food' || normalized.href === 'shanghai-food') {
      normalized.href = '/food'
      normalized.slug = '/food'
    }
    if (normalized.name === '上海美食' || normalized.name === '上海美食地图') {
      normalized.name = '上海吃饭清单'
    }
    if (normalized.title === '上海美食' || normalized.title === '上海美食地图') {
      normalized.title = '上海吃饭清单'
    }
    if (normalized.summary === '上海美食地图入口。') {
      normalized.summary = '上海吃饭清单入口。'
    }
    if (Array.isArray(normalized.subMenus)) {
      normalized.subMenus = normalizeFoodMenuLinks(normalized.subMenus)
    }
    return normalized
  })
}
