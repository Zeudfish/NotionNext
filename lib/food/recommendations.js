...
export const getRecommendationStars = recommendation => {
  const count = RECOMMENDATION_STARS[normalizeRecommendation(recommendation)]
  return count ? '★'.repeat(count) : '未分级'
}

export const getRecommendationOptionLabel = recommendation => {
  const normalized = normalizeRecommendation(recommendation)
  const stars = getRecommendationStars(normalized)
  return normalized ? `${normalized}（${stars}）` : '未分级'
}
